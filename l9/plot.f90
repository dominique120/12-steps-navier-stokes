...
!   * Inspired by: [EasyPlot](https://pypi.python.org/pypi/EasyPlot)
    ...
    type, public :: pyplot
        ...
        logical :: use_colormap = .false.    !! use color map module
...
! Initialize a plot
    ...
    subroutine initialize(me, grid, xlabel, ylabel, zlabel, title, legend, use_numpy, use_colormap, figsize, &
    ...
    logical,               intent(in), optional :: use_colormap    !! activate usage of colormap module
    ...
    if (present(use_colormap)) then
        me%use_colormap = use_colormap
    else
        me%use_colormap = .false.
    end if
    ...
    if (me%use_colormap) call me%add_str('import matplotlib.cm as cm')
    call me%add_str('')
...
!@note This requires `use_numpy` to be True.
    ...
    real(wp),dimension(:,:), intent (in)           :: z            !! z values (a matrix)
    character(len=*),        intent (in)           :: label        !! plot label
    character(len=*),        intent (in), optional :: linestyle    !! style of the plot line
    integer,                 intent (in), optional :: linewidth    !! width of the plot line
    real(wp),dimension(:),   intent (in), optional :: levels       !! contour levels to plot
    ...
    if (allocated(me%str)) then
        ...
        if (present(linewidth))  extras = extras//','//'linewidths='//trim(adjustl(iline))
        if (present(cmap))       extras = extras//','//'cmap='//cmap//''
        if (present(linestyle))  extras = extras//','//'linestyle="'//linestyle//'"'
